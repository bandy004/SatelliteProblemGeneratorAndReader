 ...
 */
#ifndef SATPROBREADER_H
#define SATPROBREADER_H
...
public:
    ...
    int maxEndForSatelliteGroundStation;
    ...
    {
        maxEndForSatelliteGroundStation = -1;
    }
        ...
        for (int i = 0; i < num_satellites; i++)
        {
            ...
            for (int k = 0; k < num_ground_statiion; k++)
            {
                ...
                for (int j = 0; j < tokens.size(); j++)
                {
                    ...
                    tokenize(pairstr, pairtokens, std::string("[,]"));
                    assert(pairtokens.size() == 2);
                    int min = atoi(pairtokens[0].c_str());
                    int max = atoi(pairtokens[1].c_str());
                    satellite_groundstation_visibility[i][k].push_back(std::make_pair<int, int>( min, max));
                    
                    if( max > maxEndForSatelliteGroundStation )
                    {
                        maxEndForSatelliteGroundStation = max;
                    }
                }
            }
        }
